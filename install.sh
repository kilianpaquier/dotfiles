...
if [ ! -f "$DOTFILES_CONFIG" ]; then
    log_info "Setting up default dotfiles config in $DOTFILES_CONFIG ..."
    mkdir -p "$(dirname "$DOTFILES_CONFIG")"
    cp "$dotfiles_dir/default.env" "$DOTFILES_CONFIG"
fi
log_success "Using following dotfiles configuration:"
log "$(cat "$DOTFILES_CONFIG")"
...
# INSTALLS comes from $DOTFILES_CONFIG
...
# ZSH_CUSTOMS comes from $DOTFILES_CONFIG
